<template>
	<teleport :to="popupParent">
		<div
			class="wb-input-suggestions"
			:class="popupClass?.split(' ').reduce((acc, c) => { acc[c] = true; return acc;}, {}) || {}"
			v-if="suggestionsControl.showPanel"
			v-inject-offset="{ $from: $el }"
			v-inject-size="{ $from: $el }">

			<v-navigable-selection-input
				class="-selection"
				:selection="suggestionsControl.selection"
				@selected-item="onSelectSuggestion"
				mode="single"
				ref="$suggestionsSelection"
				@focusout="suggestionsPanel_onFocusOut">
				...
				<v-navigable-selection-group
					...
					:key="groupId"
					:title="groupId"
					v-for="[groupId, suggestionsOnGroup] in groupedSuggestions">

					<!--            <span class="-group-name" v-if="groupId">{{ groupId }}</span>-->

					<v-navigable-selection-item
						class="-item"
						:item-id="suggestion.id"
						:key="suggestion.id"
						@click.capture="suggestion_onClick(suggestion)"
						v-for="suggestion in suggestionsOnGroup">
...
import WbToken from "./wb-token.mjs";
...
export default {
	...
	computed: {
		...
		/**
		 * @returns {Map<string, Suggestion[]>}
		 */
		groupedSuggestions() {
			const groupedSuggestions = new Map();
			for (let suggestion of this.suggestionsControl.suggestions) {
				if (!groupedSuggestions.has(suggestion.groupName)) {
					groupedSuggestions.set(suggestion.groupName, [suggestion]);
				} else {
					groupedSuggestions.get(suggestion.groupName).push(suggestion);
				}
			}
	...
	methods: {
		 ...
		 * @param {FocusEvent} e
		 */
		suggestionsPanel_onFocusOut(e) {
			if (e.relatedTarget != this.$el && !DomUtils.getParentUsingPredicate(e.relatedTarget, $el => $el == this.$el))
				this.$nextTick(() => {
					this.suggestionsControl.closeSuggestionsPanel();
				});
		},
		 ...
		 */
		suggestion_onClick(suggestion){
			this.promoteSuggestion(suggestion);
		},
		...
		onSelectSuggestion(suggestionId) {
			const selectedSuggestion = this.suggestionsControl.suggestions.find(s => s.id === suggestionId);
			if (!selectedSuggestion)
				throw new Error(`Invalid selectedSuggestion`);

			this.promoteSuggestion(selectedSuggestion);
		},
		...
		promoteSuggestion(suggestion) {
			const newToken = new WbToken(suggestion.id, suggestion.data);

			const newTokenIndex = this.suggestionsControl.triggeredByFakeInputIndex;
			if (newTokenIndex < 0)
				throw new Error(`Invalid triggeredByFakeInputIndex`);

			// TODO validate if can add this token on this position

			// *Clearing the fake input text:
			const triggeredByFakeInput = this.$el.querySelector(`.-fake-input[data-fake-input-index="${newTokenIndex}"]`);
			if (triggeredByFakeInput) {
				this.changeInputValue(triggeredByFakeInput, '');
				// triggeredByFakeInput.value = '';
				// triggeredByFakeInput.dispatchEvent(new Event('input'));
			}

			// *Adding the selected suggestion on the input as a new token:
			this.tokens.splice(newTokenIndex, 0, newToken);

			// *Closing the suggestions panel:
			this.suggestionsControl.closeSuggestionsPanel();

			// *Focusing back on the correct fake input:
			setTimeout(() => {
				let nextFakeInput = this.$el.querySelector(`.-fake-input[data-fake-input-index="${newTokenIndex + 1}"]`);
				if (!nextFakeInput)
					nextFakeInput = this.$el.querySelector(`.-fake-input[data-fake-input-index="${newTokenIndex}"]`);
				if (!nextFakeInput)
					nextFakeInput = this.$el;
...
.wb-input-suggestions > .-selection {
	display: flex;
